...
import { initFrontendTracing, getMeterProvider } from './FrontendTracer';

const METER_NAME = 'ssr-react-web-vitals';
...
const lastMetricValues: Map<string, { value: number; attributes: Record<string, string | number> }> = new Map();
...
export function initWebVitalsMonitoring() {
    ...
    try {
        // Initialize frontend tracing first
        initFrontendTracing();

        const meterProvider = getMeterProvider();
        if (!meterProvider) {
            console.error('[Web Vitals] MeterProvider not available');
            return;
        }
        ...
        console.info('[Web Vitals] Monitoring initialized with periodic refresh for ssr-react');
    } catch (error) {
